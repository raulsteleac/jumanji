...
from jumanji.environments.routing.lbf_empty import utils
from jumanji.environments.routing.lbf_empty.types import (
    Agent,
    Entity,
    Observation,
    State,
...
class LbfObserver(abc.ABC):
    ...
    def __init__(self, fov: int, grid_sizeX: int, grid_sizeY: int, num_agents: int, num_food: int):
        """
...
class VectorObserver(LbfObserver):
    ...
    def __init__(self, fov: int, grid_sizeX: int, grid_sizeY: int, num_agents: int, num_food: int) -> None:
        super().__init__(fov, grid_sizeX, grid_sizeY, num_agents, num_food)
        self.visualise_all_agents = True
        self.grid_size = jnp.max(jnp.array([grid_sizeX, grid_sizeY]))
...
class GridObserver(LbfObserver):
    ...
    def make_agents_view(self, state: State) -> chex.Array:
        ...
        return jnp.stack([agents_view, access_masks], axis=1)
